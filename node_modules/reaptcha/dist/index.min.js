...
var getFilteredScripts = function getFilteredScripts(scriptSrc) {
  return Array.from(document.scripts).filter(function (script) {
    return script.src.indexOf(scriptSrc) > -1;
  });
};

var isScriptPresent = function isScriptPresent(scriptSrc) {
  return getFilteredScripts(scriptSrc).length > 0;
};

/* harmony default export */ var isAnyScriptPresent = (function (scripts) {
  return scripts.reduce(function (isPresent, url) {
    return isPresent ? isPresent : isScriptPresent(url);
  }, false);
});
